...
const SVG_TRANSLATE = '<svg width="16" height="16" viewBox="-2 -2 20 20" fill="none"><text x="3" y="6" text-anchor="middle" fill="var(--itranslate-float-icon-color)" font-size="9" font-weight="700" font-family="sans-serif" transform="rotate(-15, 3, 6)">A</text><text x="13" y="6" text-anchor="middle" fill="var(--itranslate-float-icon-color)" font-size="9" font-weight="700" font-family="sans-serif" transform="rotate(15, 13, 6)">あ</text><text x="8" y="17" text-anchor="middle" fill="var(--itranslate-float-icon-color)" font-size="8" font-weight="700" font-family="sans-serif">文</text></svg>';
const SVG_UNDO = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--itranslate-float-icon-color)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 10h10a5 5 0 0 1 0 10H9"/><path d="M7 6l-4 4 4 4"/></svg>';
const SVG_SELECTION = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--itranslate-float-icon-inactive)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="itranslate-float-selection-icon"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>';
...
export function createFloatingPanel(actions: PanelActions): void {
  ...
  // Initialize icon stroke via CSS variable (not hardcoded SVG attribute)
  const icon = panelEl.querySelector('.itranslate-float-selection-icon') as SVGElement;
  if (icon) icon.style.setProperty('stroke', 'var(--itranslate-float-icon-inactive)');
}
...
export function setSelectionState(enabled: boolean): void {
  ...
  if (icon) (icon as SVGElement).style.setProperty('stroke', enabled ? 'var(--itranslate-float-icon-active)' : 'var(--itranslate-float-icon-inactive)');
}
