...
**本章目标**：读完本章，你将理解 iTranslate 如何用一套 CSS 变量体系在四个隔离的扩展上下文中统一视觉风格，以及如何用 Claude Code 完成视觉打磨。你会看到 `--itranslate-*` 变量的命名和分层逻辑、极地冰川主题的设计决策、动画调试的过程，以及最重要的：**AI 在视觉设计中的正确协作模式**——它没有审美，但能帮你高效地执行审美。
