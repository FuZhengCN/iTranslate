...
**本章目标**：读完本章，你将理解 iTranslate 词典查询功能的核心——如何用 Prompt 让 AI 输出结构化的 JSON，而不是自由文本。你会看到 System Prompt 如何约束模型的输出行为、解析函数如何防御模型的"不听话"、以及 fallback 分支如何保证用户体验不被 JSON 解析失败打断。本章的重点不是词典学，而是**结构化输出——AI 编程中最重要的 Prompt Engineering 技能之一**。
...
> 下一章：第 12 章「视觉打磨：主题与动画」——如何用 CSS 变量体系统一四个上下文的视觉风格，以及 AI 辅助视觉设计的正确协作模式。
