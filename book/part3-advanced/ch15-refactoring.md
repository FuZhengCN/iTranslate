...
**本章目标**：读完本章，你将掌握用 Claude Code 安全执行复杂重构的核心方法论。AI 编程降低了重构的门槛，但同时也引入了新的风险：AI 可能在你不注意的时候"顺便"改变了行为。本章给出了五步安全重构法，配合 iTranslate 项目的两个真实重构案例，让你既能享受 AI 加速重构的红利，又能规避它带来的陷阱。
...
## 15.3 iTranslate 重构案例 1：extractor 拆分
...
## 15.4 iTranslate 重构案例 2：retry.ts 独立提取
