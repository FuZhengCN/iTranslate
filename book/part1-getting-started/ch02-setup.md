...
| Model | `deepseek-v4-flash` | DeepSeek V4 Flash，速度快、极便宜（¥1/¥2） |

日常编码用 `deepseek-v4-flash` 足够快且极省。如果遇到复杂架构设计或高难度调试，可以切到 `deepseek-v4-pro`（旗舰，¥3/¥6），推理能力更强。两个模型都支持思考模式和非思考模式，不需要像 V3 时代那样单独换一个 R1 模型。

**方式二：编辑 settings.json**

如果你更习惯直接编辑配置文件，Claude Code 的设置存储在用户目录下的 `.claude/settings.json`（全局）或项目的 `.claude/settings.json`（项目级）：
...
> 💡 在 Git Bash 中 `~/.claude/settings.json` 也能正确指向 Windows 的用户目录，所以本书统一用 `~/` 写法，跨平台通用。

编辑 `~/.claude/settings.json`（或 Windows 等效路径），添加：
...
> ⚠️ **切换模型注意**：`/config` 中切换模型后，后续所有的交互都走新的模型。日常编码用 `deepseek-v4-flash`（极速便宜），复杂任务切到 `deepseek-v4-pro`（旗舰推理）。开发过程中不需要频繁切换——绝大多数场景 V4-Flash 已经足够。
...
> - 配置 Claude Code 用 DeepSeek：`/config` 命令设置 Base URL (`https://api.deepseek.com/v1`)、API Key、模型 (`deepseek-v4-flash` 或 `deepseek-v4-pro`)。
