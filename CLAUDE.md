...
**铁律见全局 `~/.claude/CLAUDE.md`（7 条），本项目不再重复。**
...
- `npm run release` 完成后，创建 GitHub / Gitee Release 并上传 zip：
  - GitHub CLI：`gh release create vX.Y.Z iTranslate-vX.Y.Z.zip --title "iTranslate vX.Y.Z" --notes "..."`（需 `GH_TOKEN` 或 `gh auth login`）
  - Gitee API：`curl -X POST "https://gitee.com/api/v5/repos/fuzheng0312/i-translate/releases" -F "access_token=..." -F "file=@iTranslate-vX.Y.Z.zip" ...`
  - 或手动创建：GitHub https://github.com/FuZhengCN/iTranslate/releases/new / Gitee https://gitee.com/fuzheng0312/i-translate/releases/new

## Architecture

**Manifest V3 浏览器扩展**（Chrome/Edge），多语种双语翻译（默认英→中）。TypeScript 编写，Vite + `@crxjs/vite-plugin` 构建。

**权限策略：** `manifest.json` 声明 `storage`、`activeTab`、`scripting` 三项权限。`content_scripts` 在构建时由 `vite.config.ts` 中的 `inject-content-scripts` 自定义插件注入到 `dist/manifest.json`（`<all_urls>`、`run_at: document_idle`），实现页面加载时自动注入内容脚本。源 `manifest.json` 不含 `content_scripts`（避免 `@crxjs/vite-plugin` 将其当作输入入口解析）。
...
| `toggleFloatingPanel` | popup → content | 显示/隐藏浮动翻译面板。开启时自动同步按钮状态（`.itranslate-translation` 存在 → `setTranslateState('undo')`），解决"先翻译后开启面板导致按钮状态相反"的问题 |
...
用户选中文字 → mouseup
  ...
  ├─ 选区 Rect → positionBall() → createBall()  // 12px 小球在选区最后字右下角
  └─ 悬停 1s → showBubble(rect, text)
        ...
        └─ mode='translate' → 原文(textContent) + 译文(white-space:pre-line 分段)
```

小球位置逻辑（`getLastCharRect` + `positionBall`，`selection.ts`）：`getLastCharRect()` 用 `Range.collapse(false)` 缩到末尾光标位置获取最后字坐标，`positionBall()` 默认右下角（rect.bottom + 2, rect.right + 2）。底部溢出翻到上方，右侧溢出翻到左侧，左上均 clamp 2px。气泡位置逻辑（`getBubblePosition`）：优先选区下方居中 → 底部溢出翻到上方。
...
Vitest + jsdom + `fake-indexeddb` (auto-loaded via `setupFiles`)。测试文件（11 个，103 个用例）位于各模块的 `__tests__/` 目录：
...
| `src/content/__tests__/floating-panel.test.ts` | 面板创建/销毁、翻译/划词状态切换、点击回调 |
...
### Chrome Web Store 上架资料 (`docs/store/`)

| 文件 | 用途 |
|------|------|
| `privacy.html` | 隐私政策页面，通过 GitHub Pages 托管（`docs/` 目录）。Web Store 隐私权规范标签页填入此 URL：`https://fuzhengcn.github.io/iTranslate/docs/store/privacy.html` |
| `listing.md` | 商店列表文案 |
| `description.txt` | 纯文本描述 |
| `promo-large.svg` | 宣传图 |
| `screenshots/` | 截图（如已上传） |

**Web Store 审核提交要点：** 因 `content_scripts` 声明了 `<all_urls>`，隐私权规范标签页需填写：
- **单用途说明：** 内联双语网页翻译与词典查词
- **主机权限理由：** 页面加载时自动注入脚本以创建翻译悬浮操作面板；脚本仅在用户主动操作时读取页面内容，不自动读取或上传任何数据
- **隐私政策 URL：** 填入上述 GitHub Pages 链接

**更新隐私政策后重新部署 GitHub Pages：** 仓库 Settings → Pages → Branch 设为 `master`、`/docs` 目录。推送 `master` 后自动生效。
